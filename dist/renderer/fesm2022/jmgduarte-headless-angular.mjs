import { DOCUMENT, isPlatformBrowser, isPlatformServer, NgClass, NgStyle, NgComponentOutlet, NgTemplateOutlet } from '@angular/common';
import * as i0 from '@angular/core';
import { inject, Injectable, PLATFORM_ID, input, Injector, effect, computed, Component, signal, DestroyRef, InjectionToken, makeEnvironmentProviders, ViewChild, output } from '@angular/core';
...
class SafeStyleService {
    ...
    flush() {
        ...
        if (!this.styleElement) {
            this.styleElement = this.document.head.querySelector('style[data-headless-responsive]')
...
class HeadlessResourceHintService {
    document = inject(DOCUMENT);
    platformId = inject(PLATFORM_ID);
    preloadImage(key, src, srcSet, sizes) {
        if (!src || (!isPlatformBrowser(this.platformId) && !isPlatformServer(this.platformId)))
            return;
        let link = [...this.document.head.querySelectorAll('link[data-headless-preload]')]
    ...
    static ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.2.23", ngImport: i0, type: HeadlessResourceHintService, deps: [], target: i0.ɵɵFactoryTarget.Injectable });
    static ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "21.2.23", ngImport: i0, type: HeadlessResourceHintService, providedIn: 'root' });
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.2.23", ngImport: i0, type: HeadlessResourceHintService, decorators: [{
            type: Injectable,
            args: [{ providedIn: 'root' }]
        }] });
...
class HeroComponent {
    ...
    resourceHints = inject(HeadlessResourceHintService);
...
class BasicBlockComponent {
    ...
    data = computed(() => this.block().data, ...(ngDevMode ? [{ debugName: "data" }] : /* istanbul ignore next */ []));
    isPriorityImage = computed(() => this.data().fetchPriority === true, ...(ngDevMode ? [{ debugName: "isPriorityImage" }] : /* istanbul ignore next */ []));
    imageLoading = computed(() => this.isPriorityImage() ? 'eager' : 'lazy', ...(ngDevMode ? [{ debugName: "imageLoading" }] : /* istanbul ignore next */ []));
    imageFetchPriority = computed(() => this.isPriorityImage() ? 'high' : 'auto', ...(ngDevMode ? [{ debugName: "imageFetchPriority" }] : /* istanbul ignore next */ []));
    childComponent(child) {
        return this.registry.resolve(child);
    }
    responsiveClass = computed(() => `headless-responsive-${this.block().id.replace(/[^a-zA-Z0-9_-]/g, '-')}`, ...(ngDevMode ? [{ debugName: "responsiveClass" }] : /* istanbul ignore next */ []));
